...
export default class Draggable extends Component {
	...
	constructor(props) {
		super(props);
		const { buttonPosition, buttonWidth, buttonHeight, pressDrag, pressDragRelease, longPressDrag, pressInDrag, pressOutDrag, imageSource,
	...
	_positionCss = (buttonPosition,size,x,y)=>{
	...
	_dragItemCss = (buttonPosition,buttonWidth,buttonHeight,size,color,shape)=>{
		let Window = Dimensions.get('window');
		if(shape == 'circle') {
			return{
			...
			// 	height: size
			// };
			if (buttonPosition == 'right' || buttonPosition == 'left') {
				return{
					width: buttonWidth,
					height: buttonHeight,
	...
	_getTextOrImage = () =>{
		if(this.renderShape == 'image') {
			...
			return(<Image style={this._dragItemCss(this.buttonPosition,this.buttonWidth,this.buttonHeight,this.renderSize,null,'image')} source={this.imageSource}/>);
		}else{
	...
	render() {
		...
		return (
			<View style={this._positionCss(this.buttonPosition,this.renderSize,this.offsetX,this.offsetY)}>
				<Animated.View 
